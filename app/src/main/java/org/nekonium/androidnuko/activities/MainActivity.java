...
import org.nekonium.androidnuko.data.WatchWallet;
import org.nekonium.androidnuko.fragments.FragmentPrice;
import org.nekonium.androidnuko.fragments.FragmentTransactionsAll;
import org.nekonium.androidnuko.fragments.FragmentWallets;
import org.nekonium.androidnuko.interfaces.NetworkUpdateListener;
import org.nekonium.androidnuko.services.NotificationLauncher;
import org.nekonium.androidnuko.services.WalletGenService;
import org.nekonium.androidnuko.utils.AddressNameConverter;
import org.nekonium.androidnuko.utils.Dialogs;
import org.nekonium.androidnuko.utils.ExchangeCalculator;
import org.nekonium.androidnuko.utils.ExternalStorageHandler;
import org.nekonium.androidnuko.utils.OwnWalletUtils;
import org.nekonium.androidnuko.utils.Settings;
import org.nekonium.androidnuko.utils.TxCache;
...
public class MainActivity extends SecureAppCompatActivity implements NetworkUpdateListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (preferences.getLong("APP_INSTALLED", 0) == 0) {
            Intent intro = new Intent(this, org.nekonium.androidnuko.activities.AppIntroActivity.class);
            startActivityForResult(intro, org.nekonium.androidnuko.activities.AppIntroActivity.REQUEST_CODE);
        }
    ...
    public void onActivityResult(int requestCode, int resultCode, final Intent data) {
        ...
        if (requestCode == org.nekonium.androidnuko.activities.QRScanActivity.REQUEST_CODE) {
            if (resultCode == RESULT_OK) {
                byte type = data.getByteExtra("TYPE", org.nekonium.androidnuko.activities.QRScanActivity.SCAN_ONLY);
                if (type == org.nekonium.androidnuko.activities.QRScanActivity.SCAN_ONLY) {
                    if (data.getStringExtra("ADDRESS").length() != 42 || !data.getStringExtra("ADDRESS").startsWith("0x")) {
                        snackError("Invalid address!");
                        return;
                    }
                    Intent watch = new Intent(this, org.nekonium.androidnuko.activities.AddressDetailActivity.class);
                    watch.putExtra("ADDRESS", data.getStringExtra("ADDRESS"));
                    startActivity(watch);
                } else if (type == org.nekonium.androidnuko.activities.QRScanActivity.ADD_TO_WALLETS) {
                    if (data.getStringExtra("ADDRESS").length() != 42 || !data.getStringExtra("ADDRESS").startsWith("0x")) {
                        snackError("Invalid address!");
                        return;
                    }
                ...
                } else if (type == org.nekonium.androidnuko.activities.QRScanActivity.REQUEST_PAYMENT) {
                    if (WalletStorage.getInstance(this).getFullOnly().size() == 0) {
                        Dialogs.noFullWallet(this);
                    } else {
                        Intent watch = new Intent(this, org.nekonium.androidnuko.activities.SendActivity.class);
                        watch.putExtra("TO_ADDRESS", data.getStringExtra("ADDRESS"));
                        watch.putExtra("AMOUNT", data.getStringExtra("AMOUNT"));
                        startActivity(watch);
                    }
                } else if (type == org.nekonium.androidnuko.activities.QRScanActivity.PRIVATE_KEY) {
        ...
        } else if (requestCode == org.nekonium.androidnuko.activities.WalletGenActivity.REQUEST_CODE) {
            if (resultCode == RESULT_OK) {
        ...
        } else if (requestCode == org.nekonium.androidnuko.activities.SendActivity.REQUEST_CODE) {
        ...
        } else if (requestCode == org.nekonium.androidnuko.activities.AppIntroActivity.REQUEST_CODE) {
            if (resultCode != RESULT_OK) {
                finish();
            } else {
                SharedPreferences.Editor editor = preferences.edit();
        ...
        } else if (requestCode == org.nekonium.androidnuko.activities.SettingsActivity.REQUEST_CODE) {
    ...
    public void importPrivateKey(String privatekey) {
        Intent genI = new Intent(this, org.nekonium.androidnuko.activities.WalletGenActivity.class);
        genI.putExtra("PRIVATE_KEY", privatekey);
        startActivityForResult(genI, org.nekonium.androidnuko.activities.WalletGenActivity.REQUEST_CODE);
    }
    ...
    private void selectItem(int position) {
        switch (position) {
            ...
            case 2: {
                Intent settings = new Intent(this, org.nekonium.androidnuko.activities.SettingsActivity.class);
                startActivityForResult(settings, org.nekonium.androidnuko.activities.SettingsActivity.REQUEST_CODE);
                break;
            }
            case 3: {
